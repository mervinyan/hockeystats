...
router.get('/:streamid', function (req, res, next) {
    ...
    fetch_events_backwards(streamid, -1, 1000, true, function (readResult) {
        ...
        for (var i = 0; i < readResult.Events.length; i++) {
            ...
            game_events[i] = { type: event.EventType, json: JSON.parse(bin2String(event.Data.toJSON().data)) };
        }
...
router.post('/homescore', function (req, res, next) {
    append_event(req, res, validateScoreRequest, extractScoreEventData, 'GoalScored');

});


router.post('/homepenalty', function (req, res, next) {
    append_event(req, res, validatePenaltyRequest, extractPenaltyEventData, 'PenaltyTaken');
});


router.post('/guestscore', function (req, res, next) {
    append_event(req, res, validateScoreRequest, extractScoreEventData, 'OpponentGoalScored');
});


router.post('/guestpenalty', function (req, res, next) {
    append_event(req, res, validatePenaltyRequest, extractPenaltyEventData, 'OpponentPenaltyTaken');
});
...
function append_event(req, res, requestValidator, eventDataExtractor, eventType) {
...
function fetch_events_backwards(stream, start, count, resolveLinkTos, resultProcessor) {
    ...
    connection.on('connect', function () {
        console.log('connecting to geteventstore...');
        connection.readStreamEventsBackward(stream, { start: start, count: count, resolveLinkTos: count }, function (err, readResult) {
            if (err) return console.log('Ooops!', err);
